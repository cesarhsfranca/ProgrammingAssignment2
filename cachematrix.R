## The below code demonstrates the concept of lexical scoping and cache. Scoping is the mechanism within R that determines how R finds symbols (i.e. programming language elements, data objects like for example “x” and “m” as well as other data objects and functions that can be defined in the body of function, for example) to retrieve their values during the execution of an R script.
...
## A cache is a way to store objects in memory to accelerate subsequent access to the same object. In statistics, some matrix algebra computations are notoriously expensive in terms of time, such as calculating the inverse of a matrix. Therefore, if one needs to use the same inverted matrix for subsequent computations, it is advantageous to cache it in memory instead of repeatedly calculating the inverse. 
...
## Note: For the below code and according to the assignment the matrix supplied is always invertible which means its determinant is different than zero otherwise its inverse cannot be calculated and an error would be generated (is not the intent of this assignment to describe it in more details. Please in the case it is necessary “Google” how to invert a matrix and/or invertible matrix).

## 2.	cacheSolve(): computes the inverse of the special “matrix” created by the function makeCacheMatrix(). Based on that if the inverse has been already calculated and not changed the cacheSolve() retrieves the inverse of the matrix from the cache. 
...
##Initialization of objects with a default value is important ##because without a default value, data <- x$get() generates the ##following error message.
...
## Since inv is defined in a parent environment and it needs to ## be accessed after the execution of set(), the <<- form of the 
## assignment operator is used to assign the inv argument into ## the parent environment.

        setinv = function(inverse) inv <<- inverse 

## Here is defined the getter for the inv where R takes  
## advantage of the Lexical Scoping to find the correct symbol ## inv to retrieve its value.

        getinv = function() inv

## At this moment the getters and setters are defined for both ## of data objects within the function makeCacheMatrix ().
## In the final line of code in the makeCacheMatrix () assigns ## each created function as an element in a list () to be  
...
## function.                                              
## The reason to name the list elements is to allow the use of ## the $ form of extract operator in order to access the 
